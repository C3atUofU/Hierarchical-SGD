...
if __name__ == '__main__':
    ...
    #filename
    f_l1 = args.dataset+'-'+str(args.num_groups)+'-'+str(args.group_freq)+'-'+str(args.local_period)+'-loss.txt'
    f_a1 = args.dataset+'-'+str(args.num_groups)+'-'+str(args.group_freq)+'-'+str(args.local_period)+'-acc.txt'
    f_l2 = args.dataset+'-'+str(args.num_groups)+'-'+str(args.group_freq)+'-'+str(args.local_period)+'-test_loss.txt'
    f_a2 = args.dataset+'-'+str(args.num_groups)+'-'+str(args.group_freq)+'-'+str(args.local_period)+'-test_acc.txt'
        ...
        for group_idx in range(args.num_groups):
            ...
            for i in range(0,group_epochs[group_idx]):
                ...
                for j in range(0, len(sampled_clients)):
                    ...
                    local = LocalUpdate(args=args, dataset=dataset_train, idxs=dict_users[idx], iters = local_iters[group_idx], nums=num_samples[idx])
            ...
            if w_glob is None:
                ...
                for k in w_group.keys():
                    w_glob[k] += w_group[k] * gweight[group_idx]
